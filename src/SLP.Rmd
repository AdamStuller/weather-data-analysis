...
# SLP stlpec EDA
## Atmosfericky tlak v hektopascaloch
...
median(mandatory_data$SLP, na.rm = TRUE)
mean(mandatory_data$SLP, na.rm = TRUE)
``` 
...
EnvStats::cv(mandatory_data$SLP, na.rm = T) # variacny koeficient
```
...
ggplot(data = df, aes( SLP,factor(year), colour=year)) +
  geom_boxplot() 
```
### Problemy s rokom 2017

Vidime, ze za rok 2017 nie su ziadne namerane hodnoty tlkau okrem jednej jedinej (ktora tu nie je upravena).
...
Vx <- lo * cos(pi * mandatory_data$SLP)
Vy <- lo * sin(pi * mandatory_data$SLP)
