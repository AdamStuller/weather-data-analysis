...
```{r}
# Interquartile range
Q1_slp <- quantile(mandatory_data$SLP, 0.25, na.rm = T) # 25% hodnot je mensich a 75% vacsich
Q3_slp <- quantile(mandatory_data$SLP, 0.75, na.rm = T) # 75% hodnot je mensich a 25% vacsich

(IQR(mandatory_data$SLP, na.rm = T ) / 2) %>%# interquartile range
...
ggplot(data = df, aes( SLP,factor(year), colour=year)) +
  geom_boxplot() +
  labs(title = paste("Atmospheric pressure boxplot, individual years")) +
  xlab("SLP - Atmospheric pressure") +
  ylab("Year") 
```
...
ggplot(data = df, aes( pressure,factor(label), colour=label)) +
  geom_boxplot() +
  labs(title = paste("Atmospheric pressure boxplot")) +
  xlab("") +
  ylab("") 
```
