...
replace_NA <- function(val, scaling = 1) {
...
mandatory_data <- data_sliac %>%
  ...
  dplyr::mutate(
    TMP = map_dbl(.$TMP,  replace_NA, 10),
    WND_ANGLE = map_dbl(.$WND_ANGLE, replace_NA),
    WND_SPEED = map_dbl(.$WND_SPEED, replace_NA, 10),
    VIS = map_dbl(.$VIS, replace_NA),
    SLP = map_dbl(.$SLP, replace_NA, 10),
    CIG = map_dbl(.$CIG, replace_NA),
    DEW = map_dbl(.$DEW, replace_NA, 10)
  ) %>%
